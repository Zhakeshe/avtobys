...
const browserBluetooth = typeof navigator !== "undefined" ? (navigator as Navigator & { bluetooth?: { requestDevice?: (options: unknown) => Promise<{ id?: string; name?: string }> } }).bluetooth : undefined;
...
export function BluetoothScannerScreen(props: HardwareProps) {
  ...
  async function searchBluetooth() {
    if (!browserBluetooth?.requestDevice) {
      setError("Web Bluetooth браузерде қолдау таппайды.");
      return;
    }
    ...
    try {
      ...
      setValidators((current) =>
        current.some((item) => item.id === device.id)
          ? current.map((item) => (item.id === device.id ? { ...item, label, bus } : item))
          : [{ id: device.id || label, label, bus }, ...current],
      );
      setError("");
    } catch (error) {
      setError(error instanceof Error ? error.message : "Құрылғы таңдалмады");
    }
  }
  ...
  async function handleAddBus() {
    if (!selected || !routeNumber.trim() || !tariffId) return;
    const bus = await addBus({ cityId, tariffId, number: normalizeTransportValue(selected.label), routeNumber: routeNumber.trim() });
      ...
      {stage === "scan" ? (
        <>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {validators.length === 0 ? <Empty text="Bluetooth құрылғысын іздеу үшін браузер chooser ашылады." /> : null}
            {validators.map((item) => (
              <Pressable key={item.id} style={styles.listRow} onPress={() => void handleSelect(item)}>
                <View style={styles.dot}><MaterialIcons name="directions-bus" size={18} color="#FFF" /></View>
            ...
            <Pressable style={styles.ghost} onPress={() => void searchBluetooth()}><Text style={styles.ghostText}>Қайтадан іздеу</Text></Pressable>
...
export function QrScannerPaymentScreen(props: HardwareProps) {
  ...
  if (!permission.granted) return <View style={styles.screen}><Header title="QR төлем" onBack={props.onBack} /><Empty text="QR сканерлеу үшін камера рұқсаты керек." /><PrimaryButton label="Камераны қосу" disabled={false} onPress={() => void requestPermission()} /></View>;
        ...
        <View style={styles.bottom}>
          {bus ? (
            <View style={styles.sheet}>
              ...
              <Pressable style={styles.ghost} onPress={() => { setBus(null); setScanned(false); setError(""); }}><Text style={styles.ghostText}>Қайта сканерлеу</Text></Pressable>
            </View>
          ) : (
            <WalletBanner phoneNumber={props.phoneNumber} balance={props.walletBalance} />
          )}
