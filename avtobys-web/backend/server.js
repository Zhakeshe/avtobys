...
app.use(
  cors({
    origin(origin, callback) {
      ...
      if (!origin || CORS_ORIGIN_LIST.includes(origin)) {
        callback(null, true);
        return;
      }
...
const CORS_ORIGIN_LIST = readEnvString("CORS_ORIGIN", "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
let SERVICE_VERSION = readEnvString("APP_VERSION", "");
if (!SERVICE_VERSION) {
  try {
    SERVICE_VERSION = require(path.join(__dirname, "package.json")).version || "0.0.0";
  } catch {
...
/** Combined status for ops / maintenance UI: public config + service version. */
app.get("/api/status", async (_req, res) => {
  try {
    const settings = await withClient(async (client) => getAppSettings(client));
    const config = serializePublicConfig(settings);
    res.json({
      ok: true,
      now: nowIso(),
      version: SERVICE_VERSION,
